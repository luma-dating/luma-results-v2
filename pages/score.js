...
export function matchProfileWithWiggleRoom(fluency, maturity, bs, attachmentScore = 0, total = 0) {
  const scoredMatches = profileDescriptions.profiles.map((p) => {
    ...
    const categoryMatch = (() => {
      ...
      switch (p.categoryRule) {
        ...
        case 'EF<RM>BS':
          return fluency < maturity && bs < maturity;
        case 'EF=RM=BS':
          return Math.abs(fluency - maturity) <= 10 && Math.abs(maturity - bs) <= 10;
        case 'BS>RM>EF':
    ...
    return { ...p, avgDiff, gteMatch, inTotalRange, categoryMatch };
  });

  const sortedMatches = scoredMatches
    .filter((p) => p.gteMatch && p.inTotalRange && p.categoryMatch)
