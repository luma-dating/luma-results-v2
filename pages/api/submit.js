// LUMA Results Scoring API (Zapier-compatible)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Example payload from Zapier (adjust field keys as needed)
    const answers = Array.from({ length: 72 }, (_, i) => {
      const key = `Q${i + 1}`;
      return parseInt(data[key], 10);
    });

    // Reverse scored question indexes (0-based)
